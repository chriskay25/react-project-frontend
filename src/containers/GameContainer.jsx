...
class GameContainer extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      positions: {
        player: {
          x: boardSize.width / 2 - playerSize / 2,
          y: boardSize.height / 2 - playerSize / 2,
        },
        enemies: [],
      },
      up: false,
  ...
  handlePlayerMovement = (e) => {
    ...
    if (down && y < boardSize.height - playerSize && !paused) {
      this.setState({
        positions: {
          ...this.state.positions,
          player: {
    ...
    if (right && x < boardSize.width - playerSize && !paused) {
      this.setState({
        positions: {
          ...this.state.positions,
          player: {
  ...
  createNewEnemy = () => {
    ...
    if (!gameOver && !paused) {
      this.enemyCreationInterval = setTimeout(
        this.createNewEnemy,
        enemyInterval
      );
    }
      ...
      positions: {
        ...
        enemies: enemies
          ...
          .map((enemy) => {
            if (
              enemy.y < 0 ||
              enemy.y > boardSize.height - playerSize ||
              enemy.x < 0 ||
              enemy.x > boardSize.width - playerSize
            ) {
              enemy.remove = true;
              return enemy;
            }
  ...
  pauseGame = () => {
    const { gameOver, paused, score, speed, timeElapsed, enemyInterval } =
      this.state;
  ...
  render() {
    ...
    return (
      <div className="GameContainer">
        ...
        {/* <GameStats time={timeElapsed} speed={speed} enemyInterval={enemyInterval} gameOver={gameOver} /> */}
      </div>
    );
  }
...
export default connect(mapState, { saveGame, getHighScore })(GameContainer);
